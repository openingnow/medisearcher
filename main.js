...
const cstr_to_jsstr = ptr => {
    const encoded_result = new Uint8Array(exports.memory.buffer, ptr + strlen_portion_bytes, new Uint32Array(exports.memory.buffer, ptr, strlen_portion_bytes * 8 / 32)[0]);
    return new TextDecoder("utf8").decode(encoded_result);
}

const cstr_to_jsstr_free = ptr => {
    const result = cstr_to_jsstr(ptr);
    exports.free(ptr);

    return result;
}

const with_haskell_function = (f, str) => {
    const encoded_arg = new TextEncoder().encode(str);
    const arg_heap_addr = exports.malloc(encoded_arg.length + strlen_portion_bytes);
    new Uint32Array(exports.memory.buffer, arg_heap_addr).set([encoded_arg.length]);
    new Uint8Array(exports.memory.buffer, arg_heap_addr + strlen_portion_bytes).set(encoded_arg);

    return cstr_to_jsstr_free(f(arg_heap_addr));
}
...
const importObject = {
    ...
    env: {
        console_log: ptr => console.log(cstr_to_jsstr(ptr), 'via env.console_log'),
        eval: ptr => eval(cstr_to_jsstr(ptr)),
    },
}
...
exports.hs_init(0, 0);
const strlen_portion_bytes = exports.strlen_portion_bytes();
exports.main();
